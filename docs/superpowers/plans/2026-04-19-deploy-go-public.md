...
> **Scope:** Everything from "Tracks B/C/D/E merged on main" to "riskrask.com is publicly playable behind Cloudflare with Zero Trust on `admin.riskrask.com`."
...
- [ ] **3.1** Confirm domain is `riskrask.com` (or substitute) and lives in the user's Cloudflare account.
- [ ] **3.2** A record: `api.riskrask.com` → `159.69.91.90` (proxied / orange cloud ON).
- [ ] **3.3** CNAME: `www.riskrask.com` → Pages project (set in Phase 5).
- [ ] **3.4** CNAME: `admin.riskrask.com` → Pages project (set in Phase 5; Access policy added in Phase 7).
- [ ] **3.5** SSL/TLS mode: **Full (strict)**.
- [ ] **3.6** Origin certificate: generate a CF Origin cert for `*.riskrask.com`, install on the box for Caddy (Caddy's CF DNS-01 path also works — pick one). Document choice in `infra/hetzner/tls.md`.
...
- [ ] **4.4** Caddyfile: reverse-proxy `api.riskrask.com → http://127.0.0.1:8787` (HTTP and WebSocket upgrade). Health-check probe `/health`.
- [ ] **4.5** Smoke: `curl https://api.riskrask.com/health` → `{ "ok": true, ... }`.
- [ ] **4.6** Smoke WebSocket: `wscat -c wss://api.riskrask.com/ws/echo` (add a minimal echo route if needed for this test).
...
- [ ] **5.1** Pages project `riskrask-web`: connect repo, build command `bun run --filter='@riskrask/web' build`, output dir `apps/web/dist`, env vars (`VITE_API_URL=https://api.riskrask.com`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_TURNSTILE_SITE_KEY`).
- [ ] **5.2** Pages project `riskrask-admin`: same shape, output `apps/admin/dist`, env (`VITE_API_URL=https://api.riskrask.com`).
- [ ] **5.3** Wait for first build to succeed.
- [ ] **5.4** Bind custom domains: `www.riskrask.com → riskrask-web`, `admin.riskrask.com → riskrask-admin`.
- [ ] **5.5** Add a redirect `riskrask.com → www.riskrask.com` (Cloudflare bulk redirect or page rule).
- [ ] **5.6** Smoke: open `https://www.riskrask.com`, verify Console renders + connects to `api.riskrask.com`.
...
- [ ] **6.2** `infra/cloudflare/workers/save-redirect/` — Worker on `riskrask.com/r/:code` that 302s to `https://www.riskrask.com/?save=:code`. Deploy.
...
- [ ] **7.1** In Cloudflare Zero Trust dashboard, create an **Access Application**:
  ...
  - Application domain: `admin.riskrask.com`
...
- [ ] **7.5** Smoke: open `https://admin.riskrask.com` from an allowed account → land on the dashboard. From a non-allowlist account → blocked at CF.

## Phase 8 — Turnstile + ToS

- [ ] **8.1** Create a Turnstile widget for `*.riskrask.com`. Capture site key + secret.
...
- [ ] **9.4** Uptime: configure a free uptime monitor (UptimeRobot or BetterStack) on `https://api.riskrask.com/health` and `https://www.riskrask.com`.
